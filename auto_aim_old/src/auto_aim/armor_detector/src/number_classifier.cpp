 ...
 * @brief 对数字 ROI 进行透视变换并使用 ONNX DNN 对装甲数字分类。
 *
 * 职责：
 *  - 构造时加载 ONNX 模型和标签文件；
 *  - `extractNumbers`：将由两根光条定义的图像补丁变换为用于分类的规范裁剪；
 *  - `classify`：运行网络，计算 softmax 概率，填充装甲的分类字段，并过滤低置信或被忽略的类别。
 */
    ...
    std::remove_if(
      ...
      [this](const Armor & armor) {
        ...
        for (const auto & ignore_class : ignore_classes_) {
          // std::cout<<ignore_class<<std::endl;
