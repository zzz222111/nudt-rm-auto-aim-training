...
{
  // std::cout<<"!!!initial_vel!!!"<<initial_vel_<<std::endl;
  // forward motion model: gravity model (抛物型模型)
  auto forward_motion = [&](double given_angle, double given_x, double & h, double & t) {
      
      // t = given_x / (initial_vel_ * cos(given_angle));//飞行时间
      // h = initial_vel_ * sin(given_angle) * t - GRAVITY * t * t / 2;//高度

      double k1 = 0.47 * 1.169 * ( M_PI * 0.02125 * 0.02125) / 2 ;/// 0.055;  //0.041;
      // std::cout<<"given_angle"<<given_angle<<"initial_vel_"<<initial_vel_<<std::endl;
      t = (pow(2.718281828, k1 * given_x) - 1) / (k1 * initial_vel_ * cos(given_angle));
      h = initial_vel_ * sin(given_angle) * t / cos(given_angle) 
                - 0.5 * GRAVITY * t * t / cos(given_angle) / cos(given_angle);
...
bool GravityProjectileSolver::solve(double target_x, double target_h, double & angle,double vel)
{
  return iterative_tool_->solve(target_x, target_h, angle,vel);
}
